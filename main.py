...
logging.debug('Training LeNet on MNIST')
for p in vars(args).items():
    logging.debug('  ',p[0]+': ',p[1])
logging.debug('\n')
...
def train(epoch):
    ...
    for batch_ix, (data, target) in enumerate(train_loader):
        ...
        if batch_ix % 100 == 0 and batch_ix>0:
            logging.debug('[Epoch %2d, batch %3d] training loss: %.4f' %
...
def test():
    ...
    logging.debug('[Epoch %2d] Average test loss: %.3f, accuracy: %.2f%%\n'
